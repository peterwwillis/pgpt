...
- **Optimization**: The `sherpa-onnx` build uses the `Release` build type for maximum performance.
- **Binary Sizes**:
    - Standard build: ~32MB (dynamically links `libc`, `libstdc++`, `libgomp`).
    - Static build: ~34MB (`make build-static`, fully self-contained).
